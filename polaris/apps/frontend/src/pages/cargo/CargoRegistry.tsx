...
export default function CargoRegistry() {
  return (
    <div className="space-y-4 max-w-[1200px] mx-auto">
      <h1 className="text-2xl font-bold text-ice-50">Cargo Registry</h1>
          ...
          <tbody className="divide-y divide-ice-800/50">
            {rows.map(r => (
              <tr key={r.id} className="hover:bg-ice-900/30">
                ...
                <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/15 text-blue-400">{r.status}</span></td>
                <td className="px-4 py-3"><span className="text-xs text-red-400">{r.priority}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
