    ...
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <Link href="/cargo/registry"><a className="px-3.5 py-2 rounded-xl bg-ice-800/80 text-sm text-ice-300 hover:bg-ice-700 border border-ice-700">Registry</a></Link>
          <Link href="/cargo/tracking"><a className="px-3.5 py-2 rounded-xl bg-ice-800/80 text-sm text-ice-300 hover:bg-ice-700 border border-ice-700">Live Tracking</a></Link>
          <Link href="/cargo/scan">
            <a className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 text-sm text-white font-medium shadow-lg shadow-cyan-600/20 cursor-pointer inline-block">
              Scan Cargo
            </a>
          </Link>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
        ...
        <div className="px-5 py-3.5 border-b border-ice-800/50 font-semibold text-ice-100 text-sm flex items-center justify-between">
          Recent Shipments
          <Link href="/cargo/registry"><a className="text-xs text-cyan-400 font-normal flex items-center gap-1">View all <ArrowRight className="w-3 h-3" /></a></Link>
        </div>
        <div className="divide-y divide-ice-800/40">
          {recent.map((r, i) => (
            <motion.div key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 + i * 0.04 }}
